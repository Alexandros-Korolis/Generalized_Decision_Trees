...
data = read.csv("Indian Liver Patient Dataset (ILPD).csv", sep = ",")
# check structure
str(data)
# Change column names 
colnames(data) = c("Age","Gender","Total Bilirubin",
                   "Direct Bilirubin","Alkaline Phosphotase",
                   "Alamine Aminotransferase","Aspartate Aminotransferase",
                   "Total Proteins","Albumin","Albumin and Globulin Ratio","Class")
# Convert Class as factor
data$Class = as.factor(data$Class)

# Check class balance
# Diagnosis:
# 1 = does not suffer from liver disease
# 2 = does suffer from liver disease
...
saveWorkbook(work_book, "repeated_holdout_cross_val_50_Indian_Liver_Patient.xlsx", overwrite = TRUE)


################################################################################ accuracy
# load results data 
results_shannon = read.xlsx("repeated_holdout_cross_val_50_Indian_Liver_Patient.xlsx",
                            sheet = "Shannon")

results_renyi = read.xlsx("repeated_holdout_cross_val_50_Indian_Liver_Patient.xlsx",
                          sheet = "Renyi")

results_sh_mit = read.xlsx("repeated_holdout_cross_val_50_Indian_Liver_Patient.xlsx",
                           sheet = "Sharma_Mittal")

results_sh_tan = read.xlsx("repeated_holdout_cross_val_50_Indian_Liver_Patient.xlsx",
                           sheet = "Sharma_Taneja")

results_tsallis = read.xlsx("repeated_holdout_cross_val_50_Indian_Liver_Patient.xlsx",
                            sheet = "Tsallis")

results_kapur = read.xlsx("repeated_holdout_cross_val_50_Indian_Liver_Patient.xlsx",
                          sheet = "Kapur")

results_kaniad = read.xlsx("repeated_holdout_cross_val_50_Indian_Liver_Patient.xlsx",
                           sheet = "Kaniadakis")

index = read.xlsx("repeated_holdout_cross_val_50_Indian_Liver_Patient.xlsx",
                  sheet = "index")
