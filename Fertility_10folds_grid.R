...
data = read.xlsx("Fertility.xlsx")
# check data structure
str(data)
# Map season to corresponding values
data$season = ifelse(data$season == "-0.33","Spring",
                     ifelse(data$season == "-1","Winter",
                            ifelse(data$season == "0.33","Summer","Fall")))
# Map Child diseases to corresponding values
data$child.disease = as.character(data$child.disease)
data$child.disease = ifelse(data$child.disease == "0","Yes","No")
# Map Accident to corresponding values
data$accident = as.character(data$accident)
data$accident = ifelse(data$accident == "0","Yes","No")
# Map Surgical Intervention to corresponding values
data$surgical_intervention = as.character(data$surgical_intervention)
data$surgical_intervention = ifelse(data$surgical_intervention == "0","Yes","No")
# Map High Fevers to corresponding values
data$high_fevers = as.character(data$high_fevers)
data$high_fevers = ifelse(data$high_fevers == "-1","<3 months ago",
                          ifelse(data$high_fevers == "0",">3 months ago","No"))
# Map Alcohol to corresponding values
data$alcohol = ifelse(data$alcohol == "0.2","Several Times a day", 
                      ifelse(data$alcohol == "0.4","Everyday", 
                             ifelse(data$alcohol == "0.6","Several times a week",
                                    ifelse(data$alcohol == "0.8","Once a Week","Hardly ever or never"))))
# Map Smoking Habit to corresponding values
data$smoking = as.character(data$smoking)
data$smoking = ifelse(data$smoking == "-1","Never",
                      ifelse(data$smoking == "0","Occasional","Daily"))
# Convert Age and hrs_sitting to numeric
data$age = as.numeric(data$age)
data$hrs_sitting = as.numeric(data$hrs_sitting)
# Convert Class to 0 and 1 
# N = Normal -> 1
# O = Altered -> 0
data$diagnosis = ifelse(data$diagnosis == "N", "1","0")
# Change column names 
colnames(data) = c("season","age","child.disease","accident",             
...
saveWorkbook(work_book, "repeated_holdout_cross_val_50_Fertility.xlsx", overwrite = TRUE)


################################################################################ accuracy
# load results data 
results_shannon = read.xlsx("repeated_holdout_cross_val_50_Fertility.xlsx",
                            sheet = "Shannon")

results_renyi = read.xlsx("repeated_holdout_cross_val_50_Fertility.xlsx",
                          sheet = "Renyi")

results_sh_mit = read.xlsx("repeated_holdout_cross_val_50_Fertility.xlsx",
                           sheet = "Sharma_Mittal")

results_sh_tan = read.xlsx("repeated_holdout_cross_val_50_Fertility.xlsx",
                           sheet = "Sharma_Taneja")

results_tsallis = read.xlsx("repeated_holdout_cross_val_50_Fertility.xlsx",
                            sheet = "Tsallis")

results_kapur = read.xlsx("repeated_holdout_cross_val_50_Fertility.xlsx",
                          sheet = "Kapur")

results_kaniad = read.xlsx("repeated_holdout_cross_val_50_Fertility.xlsx",
                           sheet = "Kaniadakis")

index = read.xlsx("repeated_holdout_cross_val_50_Fertility.xlsx",
                  sheet = "index")
